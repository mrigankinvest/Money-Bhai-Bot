...
async def confirm_default_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with get_session() as session:
        if choice == 'default_wallet_yes':
            cash_wallet = await db_writer.get_wallet_by_name(user_id, "Cash")
            if not cash_wallet:
                cash_wallet = await db_writer.create_wallet(user_id, "Cash", category="Expense")
            
            added_txn = await db_writer.add_transaction(user_id, transaction_data, cash_wallet.id)
...
async def receive_wallet_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def handle_new_wallet_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def add_txn_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        ...
        await reply_and_log(update, context, "Okay, the transaction has been cancelled.")
...
async def handle_assignment_strategy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with get_session() as session:
        if choice == "same":
            ...
            # This function now needs the update and context to be passed to it
            # since it will call the session itself.
            return await assign_individual_wallet_step(update, context)
...
async def assign_single_wallet_to_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def assign_individual_wallet_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with get_session() as session:
        ...
        # edit_text might not be available on update.message, using reply_and_log for safety
        if query:
            await query.edit_message_text(prompt, reply_markup=InlineKeyboardMarkup(keyboard))
        else:
            await reply_and_log(update, context, prompt, reply_markup=InlineKeyboardMarkup(keyboard))
