...
# ===================================================================
# Private Helper Functions for each action
# ===================================================================
...
async def _handle_add_transaction(update, context, session, data):
    ...
    # Return a confirmation object/dict instead of just a string
    return {"type": "add_transaction", "txn": added_txn}

# --- NEW: Handler for "show last transaction" ---
async def _handle_query_last(update, context, session, data):
    ...
    """
    # Instead of checking context, we query the database directly.
    txn = await db_writer.get_last_transaction(session, update.effective_user.id)
    
    if not txn:
        # This message is now only sent if the user has NO transactions at all.
        return "Pehle koi transaction add karo, bhai."
    
    # Remember this transaction's ID for immediate follow-up edits
    context.user_data['last_transaction_id'] = txn.id
    
    await reply_and_log(update, context, format_transaction_details(txn))
    return None # Indicates the action is complete.

# --- NEW: Handler for "change its category", "amount was 150", etc. ---
async def _handle_contextual_edit(update, context, session, data):
    ...
    if updated_txn:
        # Forget the transaction ID after a successful edit to prevent accidental re-edits
...
async def _handle_transfer(update, context, session, data):
    ...
    # NOTE: You might want to add balance check logic here
...
async def _handle_view_transactions(update, context, session, data):
    ...
    """
    # Default to showing 5 transactions if count is not specified
    ...
    # We need to find a wallet_id and category to build the "Back" button correctly.
    # We can just use the wallet from the last transaction in the list.
    ...
    # We can reuse our interactive list view! We'll just pass a dummy period like 'all'.
    # This will enter the interactive view, but pagination will be based on the count.
    await send_interactive_txn_list(
        ...
        period="all_recent", # A special identifier for this type of view
        page=1
    )
    return None # Signal that the action is complete.
...
ACTION_HANDLERS = {
    ...
    # "update_balance": _handle_update_balance, # Add more handlers as needed
}
...
@with_db_session
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE, session) -> int:
    # This main function uses the ACTION_HANDLERS dictionary. Its core logic doesn't need
    # to change because the new features are handled by the new helper functions.
    # The full, correct code is provided here for completeness.
